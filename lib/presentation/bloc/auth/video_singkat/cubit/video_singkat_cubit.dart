...
class VideoSingkatCubit extends Cubit<VideoSingkatState> {
  ...
  void postVideoSingkatFirestore(String title, String description,
      String tiktokUrl, String destination, File file) async {
    emit(PostVideoSingkatLoading());
    final result = await postVideoSingkat.execute(
        title, description, tiktokUrl, destination, file);
    result.fold(
      (l) => emit(VideoSingkatError(l.message)),
      (r) => emit(PostVideoSingkatSuccess()),
    );
  }
}
